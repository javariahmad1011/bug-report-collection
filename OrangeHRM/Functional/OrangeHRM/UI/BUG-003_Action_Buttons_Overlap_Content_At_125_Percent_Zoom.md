...
When the OrangeHRM application is displayed at 125% browser zoom, action buttons on employee-management pages overlap adjacent content instead of maintaining appropriate spacing and alignment.
...
Preconditions
Login using a valid OrangeHRM Administrator account.
Navigate to PIM → Employee List.
Browser zoom must initially be set to 100%.
Browser window must be maximized.
Test Data
...
Navigate to the OrangeHRM Demo application.
Login using a valid Administrator account.
Navigate to PIM → Employee List.
Verify that the page displays correctly at 100% browser zoom.
Increase the browser zoom level to 125%.
Scroll through the Employee List page.
Observe the positioning of action buttons and surrounding page elements.
Actual Result

At 125% browser zoom, action buttons overlap or appear too close to adjacent page content.

Spacing between UI elements is inconsistent compared with the layout displayed at 100% zoom.
